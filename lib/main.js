...
function uncurryRight( fcn, arity, thisArg ) {
	...
	var len;
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1Y53c', fcn ) );
	}
	if ( arguments.length < 2 ) {
		...
		if ( !isPositiveInteger( len ) ) {
			throw new TypeError( format( '1Y5Az', len ) );
		}
	}
	...
	function uncurried() {
		...
		if ( len ) {
			if ( len > arguments.length ) {
				throw new Error( format( '1Y5Bs', len, arguments.length ) );
			}
			for ( i = len-1; i >= 0; i-- ) {
				if ( isFunction( f ) ) {
					f = f.call( context, arguments[ i ] );
				} else {
					throw new Error( format( '1Y5Bv', len, i ) );
				}
			}
			return f;
		}
		for ( i = arguments.length-1; i >= 0; i-- ) {
			if ( isFunction( f ) ) {
				...
				throw new Error( format( '1Y5Bu', arguments.length, i ) );
			}
		}
		return f;
	}
